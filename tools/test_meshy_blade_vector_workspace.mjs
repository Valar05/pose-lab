...
assert(data.attachmentSnapshots?.fps?.attachment?.rotationDeg?.join(',') === '-179.998,-4.747,111.678', 'FPS snapshot should use current accepted saber rotation');
assert(data.attachmentSnapshots?.fps?.proxy?.modelLocalOffset?.join(',') === '0.00424,-0.0167,0.01744', 'FPS snapshot should use current accepted saber offset');
