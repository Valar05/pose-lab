...
assert(js.includes('legacyWeaponSocketLocalTransform') && js.includes('proxy.config.modelLocalOffset') && js.includes('position.add(new THREE.Vector3().fromArray(proxy.config.modelLocalOffset))'), 'weapon socket should preserve authored model-space offsets while calibrating the right-hand FK rest');
assert(js.includes('calibrateRightHandWeaponFk') && js.includes('proxy.rightHandFkRest') && js.includes('proxy.rightHand.worldToLocal(worldPosition.clone())'), 'Meshy right-hand socket should store a fixed RightHand-local FK transform from the accepted T-pose calibration');
assert(js.includes('applyRightHandWeaponFk') && js.includes('proxy.root.parent !== proxy.rightHand') && js.includes('proxy.root.position.copy(proxy.rightHandFkRest.position)'), 'Meshy right-hand socket should apply boring FK under RightHand after calibration');
...
assert(profiles.includes("clipTag: 'FPS-VISUAL-IK-GOLDEN'") && profiles.includes("rightRollOffsetDeg: 0") && profiles.includes("leftRollOffsetDeg: -90") && profiles.includes("leftHandBone: 'LeftHand'"), 'Meshy Ready should use the current FPS-VISUAL-IK-GOLDEN roll split and keep a Meshy socket');
...
console.log(JSON.stringify({ checked: ['meshy-visual-ik-golden-ready', 'real-sabre-weapongrip-attachment', 'manifested-sabre-provenance'] }, null, 2));
