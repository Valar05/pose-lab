...
assert(profiles.includes('scale: 0.47493') && profiles.includes('rotationDeg: [90, 0, -55.145]'), 'Meshy weapon attachment should use the hard-won manual rig-local Meshy rotation');
...
assert(js.includes("from './weapon-runtime-rules.mjs?v=pose-editor-180'"), 'browser runtime should share canonical weapon socket math');
assert(!profiles.includes('hiltTposeHandLocal') && !weaponRules.includes('hiltTposeHandLocal'), 'Meshy weapon FK must not use a separate T-pose hilt target');
assert(weaponRules.includes('config.modelLocalOffset') && weaponRules.includes('local.add(vectorFromArray(THREE, config.modelLocalOffset))'), 'weapon socket should support model-space offsets for screenshot-directed placement');
assert(weaponRules.includes('socketParent.position.copy(vectorFromArray(THREE, config.handLocalOffset))') && weaponRules.includes('syntheticGripLocal.add(vectorFromArray(THREE, config.modelLocalOffset))') && weaponRules.includes('socketParent.getWorldScale') && weaponRules.includes('proxy.root.position.copy(proxy.syntheticFkLocalPosition);') && !weaponRules.includes('socketParent.worldToLocal(targetWorld.clone())'), 'Meshy synthetic WeaponR should keep handLocalOffset local to the hand while WeaponGrip remains a stable displaced local child under WeaponR');
...
assert(weaponRules.includes('fallbackVisible = !(proxy?.model && proxy?.attachmentConfig?.url)') && weaponRules.includes('fallbackHiddenWithRealWeapon'), 'real Meshy sabre should hide fallback weapon geometry instead of drawing the fake yellow/orange bar');
