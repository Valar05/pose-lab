...
assert(js.includes("const rightHandFk = !sourceSocket && rightHand && (root.userData.positionMode || 'right-hand') === 'right-hand'") && js.includes('else if (rightHandFk) rightHand.add(root)'), 'Meshy right-hand WeaponGrip should be parented directly under RightHand for boring FK');
assert(js.includes("if ((proxy.config.positionMode || 'two-hand-center') === 'right-hand' && proxy.rightHand)") && js.includes('if (proxy.root.parent !== proxy.rightHand) proxy.rightHand.add(proxy.root)'), 'runtime should repair Meshy WeaponGrip back under RightHand instead of solving from world space');
assert(js.includes('proxy.root.position.add(new THREE.Vector3().fromArray(proxy.config.handLocalOffset))') && js.includes('proxy.root.position.add(new THREE.Vector3().fromArray(proxy.config.modelLocalOffset))'), 'Meshy WeaponGrip local offset should be the authored hand/model/grip sum under RightHand');
