...
assert(js.includes('Meshy character plus direct editable real Meshy sabre mesh'), 'bench must default to direct editable real Meshy character plus real sabre mesh');
...
assert(!js.includes('rightHand.add(weaponGrip)'), 'bench startup must not hide the real sabre behind inherited hand/FK parenting');
assert(js.includes('sabreRoot = fallbackSabreRoot') && js.includes('sabreRoot.add(sabreMesh)'), 'real sabre mesh should live under the direct editable sabre root');
...
assert(js.includes('frameObject(sabreMesh)'), 'bench must frame the loaded real sabre mesh directly');
