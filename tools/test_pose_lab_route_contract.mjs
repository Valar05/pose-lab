...
assert(route.route?.authoritativeEvidence === 'blender-authoring -> pose-lab-import -> firebase-hosted-cloud-browser', 'weapon-fk route should use the reconciled visual authority ladder');
assert(route.route?.commands?.some((entry) => entry.command.includes('pose_lab_recovery_gate.mjs')), 'weapon-fk route must start with recovery gate');
assert(route.route?.commands?.some((entry) => entry.command.includes('test_blender_authoring_contract.mjs')), 'weapon-fk route must validate Blender authoring contract');
...
assert(route.route?.requiredArtifacts?.includes('authoring/meshy_saber/exports/meshy_ready_saber_contract.json'), 'weapon-fk route must require Blender authoring contract');
...
assert(route.route?.acceptance?.includes('Blender export contract is human-approved'), 'weapon-fk route must require human-approved Blender export');
assert(route.route?.acceptance?.includes('Pose Lab import of the Blender contract is verified'), 'weapon-fk route must require verified Pose Lab import');
...
assert(doc.includes('Blender viewport approval is authoring truth') && doc.includes('Source-string tests are support-only'), 'evidence protocol should document the reconciled authority ladder and support-only string tests');
