...
assert(js.includes('const origin = frame.object.getWorldPosition(new THREE.Vector3())') && js.includes('frame.object.worldToLocal(origin.clone().add(worldDelta))') && js.includes('return end.sub(start);'), 'weapon Move must convert world deltas with full frame matrix scale, not inverse quaternion only');
assert(!js.includes('return worldDelta.clone().applyQuaternion(worldQuaternionOf(frame.object).invert())'), 'weapon Move must not ignore parent/world scale when authoring offsets');
