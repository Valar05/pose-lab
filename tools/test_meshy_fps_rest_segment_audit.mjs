...
assert(audit.handRolls[0].rollOffsetDeg === 0, `T-pose bridge should not apply the dirty ready -120 roll, got ${audit.handRolls[0].rollOffsetDeg}`);
...
assert(audit.metrics.maxHandRollAfterDeg <= 0.1, `T-pose bridge should stay source-axis aligned after segment solve, got ${audit.metrics.maxHandRollAfterDeg}`);
assert(audit.metrics.maxHandRollAppliedDeg <= 6, `T-pose bridge should apply only measured rest-basis delta, got ${audit.metrics.maxHandRollAppliedDeg}`);
...
assert(profiles.includes('restSegmentCorrection: meshyFpsRestSegmentCorrection(0)'), 'Meshy FPS T-pose calibration should not use the dirty ready -120 hand-roll offset');
...
console.log(JSON.stringify({ checked: ['meshy-fps-rest-segment-audit', 'world-position-rest-delta', 'runtime-rest-segment-correction', 'tpose-bridge-no-dirty-roll'], metrics: audit.metrics }, null, 2));
