...
assert(audit.handRolls[0].rollOffsetDeg === 0, `right-hand roll offset should be neutral, got ${audit.handRolls[0].rollOffsetDeg}`);
...
assert(audit.metrics.maxHandRollAfterDeg <= 0.1, `neutral right-hand roll should preserve source-axis alignment, got ${audit.metrics.maxHandRollAfterDeg}`);
assert(audit.metrics.maxHandRollAppliedDeg < 5, `neutral right-hand roll should not apply a large visual roll offset, got ${audit.metrics.maxHandRollAppliedDeg}`);
...
console.log(JSON.stringify({ checked: ['meshy-fps-rest-segment-audit', 'world-position-rest-delta', 'runtime-rest-segment-correction', 'neutral-right-hand-roll'], metrics: audit.metrics }, null, 2));
