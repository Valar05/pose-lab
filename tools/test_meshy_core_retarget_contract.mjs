...
assert(profiles.includes("startupClip: { name: '0T-Pose -> meshyCharacter [FPS-REST-ARMS no right roll]' }"), 'Meshy startup should use the accepted FPS/Meshy T-pose calibration');
...
assert(profiles.includes("RestProbe: ['0T-Pose -> meshyCharacter [FPS-REST-ARMS no right roll]', '0T-Pose -> meshyCharacter:FPS-REST-ARMS-CAL-0'"), 'Meshy RestProbe should default to the exact accepted neutral clip path');
assert(profiles.includes("SwordReady: ['0T-Pose -> meshyCharacter [FPS-REST-ARMS no right roll]'"), 'Meshy SwordReady should expose only the accepted T-pose baseline until Ready is promoted');
...
assert(profiles.includes("originPrefix: 'mapped-arms:player->meshyCharacter:FPS-REST-ARMS-CAL-0'"), 'Meshy RestProbe clip should preserve the neutral origin path');
assert(!profiles.includes('defaultRestClip') && !js.includes('applyDefaultModelRestClip') && !js.includes('this.applyDefaultModelRestClips();'), 'accepted neutral RestProbe must remain a generated clip, not mutate actor modelRestPose');
assert(!profiles.includes('...[-150') && !profiles.includes('FPS-REST-ARMS-CAL-120') && !profiles.includes('FPS-REST-ARMS-CAL-90') && !profiles.includes('FPS-REST-ARMS-CAL--90'), 'Meshy should not expose rejected positive or sweep hand-roll calibration clips after accepting neutral roll');
