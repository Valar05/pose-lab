...
assert(!profiles.includes("clipTag: 'FPS-VISUAL-IK-READY'") && profiles.includes("clipTag: 'FPS-SWORD-UPPER'"), 'Meshy should use the restored FPS-SWORD-UPPER Ready review path and keep Visual-IK quarantined');
assert(profiles.includes('modelLocalOffset: [1.01462, 12.80195, -0.47992]') && profiles.includes('gripLocalPosition: [0.6535, -0.02302, -0.07317]') && profiles.includes('rotationDeg: [0, 0, 0]') && profiles.includes('rotationDeg: [-67.582, 76.718, -90.52]'), 'Meshy visible hilt placement and mesh-layer rotation must remain locked to the T-pose-authored FK calibration');
...
assert(!profiles.includes("clipTag: 'FPS-VISUAL-IK-READY'") && profiles.includes("clipTag: 'FPS-SWORD-UPPER'"), 'Meshy should keep the Visual-IK Ready candidate quarantined and expose FPS-SWORD-UPPER for Ready review');
