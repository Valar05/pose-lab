...
assert(profiles.includes("startupClip: { name: 'OneHandReady -> meshyCharacter [FPS-SWORD-UPPER]' }"), 'Meshy startup should use the FPS sword ready pose');
...
assert(profiles.includes("clipNames: [\n          'OneHandReady',\n        ]"), 'Meshy retarget should generate only FPS OneHandReady for this slice');
...
assert(html.includes('./src/rig-profiles.js?v=pose-editor-99'), 'HTML should load current rig profile token');
assert(html.includes('./src/pose-lab.js?v=pose-editor-99'), 'HTML should load current runtime token');
