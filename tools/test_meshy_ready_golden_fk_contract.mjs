...
assert(poseLab.includes("worldJointProjection: spec.worldJointProjection || {}"), 'Ready helper should receive profile-owned world-joint projection config');
...
assert(readyRuntime.includes('const projection = options.worldJointProjection || {}'), 'Ready helper should read roll calibration from profile-owned projection config');
assert(readyRuntime.includes('const rightRollOffsetDeg = Number(projection.rightRollOffsetDeg ?? 0)'), 'Ready helper should default but not hardcode the current right roll');
assert(readyRuntime.includes('const leftRollOffsetDeg = Number(projection.leftRollOffsetDeg ?? -90)'), 'Ready helper should default but not hardcode left roll -90');
