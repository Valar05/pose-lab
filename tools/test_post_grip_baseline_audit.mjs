...
assert(data.attachmentSnapshots?.meshy?.attachment?.gripLocalPosition?.join(',') === '0.6535,-0.02302,-0.07317', 'Meshy snapshot should preserve the visible hilt candidate');
...
assert(profiles.includes('gripLocalPosition: [0.6535, -0.02302, -0.07317]'), 'audit must not replace the Meshy visible hilt placement');
