...
assert(manifest.authority === 'human-approved Blender viewport', 'Blender manifest must make viewport approval authoritative');
assert(manifest.status === 'scaffolded-no-approved-export', 'Blender lane must not pretend an approved export exists yet');
assert(manifest.blender?.preferredHost === 'THECAULDRON', 'Blender lane should prefer the known-good THECAULDRON exporter');
...
assert(readme.includes('Blender is the manual visual authoring surface'), 'README must state Blender-first authority');
...
console.log(JSON.stringify({
  checked: ['blender-authoring-lane', 'boring-fk-hierarchy', 'human-approval-required', 'generated-exports-ignored'],
