...
assert(js.includes('&& checks.socketStableInHand') && js.includes('&& checks.socketQuaternionStableInHand') && !js.includes('&& checks.visibleConfiguredGripAtHand') && !js.includes('&& checks.visibleAppliedHiltAtHand') && js.includes('&& checks.appliedHiltPinnedToAuthoredSocket') && js.includes('&& checks.readyHandOrientationSane') && js.includes('&& checks.visibleAppliedHiltMarker'), 'visual follow pass/fail should require FK-local weapon hierarchy stability plus sane visible hand/grip orientation');
