...
assert(js.includes('configuredGripScreenMetricPresent') && js.includes('appliedHiltScreenMetricPresent') && js.includes('appliedHiltPinnedToAuthoredSocket') && js.includes('socketStableInSourceSocket') && js.includes('socketQuaternionStableInSourceSocket') && js.includes('displayQuaternionStableInSocket'), 'visual follow should check FK-local position and rotation stability under WeaponR/WeaponGrip');
...
assert(js.includes("intendedPinTarget: 'stable FK child transform under WeaponR'") && js.includes('palmTargetReportedAsDiagnosticOnly: true') && js.includes('handBaselineReportedAsDiagnosticOnly: true') && js.includes('acceptsFallbackHiltAsProof: false'), 'live hilt state should separate FK acceptance from palm/baseline/fallback hilt visuals');
...
assert(js.includes("socketStableInHand: architecture === 'source-socket' || architecture === 'synthetic-source-socket' ? true : relativeDrift.socketInHand < 0.005") && js.includes("socketMayAnimateInHand: architecture === 'source-socket' || architecture === 'synthetic-source-socket'"), 'visual follow should allow authored synthetic WeaponR motion relative to raw hand');
assert(js.includes('&& checks.socketStableInHand') && !js.includes('&& checks.visibleConfiguredGripAtHand') && !js.includes('&& checks.visibleAppliedHiltAtHand') && js.includes('&& checks.appliedHiltPinnedToAuthoredSocket') && js.includes('&& checks.socketStableInSourceSocket') && js.includes('&& checks.socketQuaternionStableInSourceSocket') && js.includes('&& checks.visibleAppliedHiltMarker'), 'visual follow pass/fail should require FK-local weapon hierarchy stability');
