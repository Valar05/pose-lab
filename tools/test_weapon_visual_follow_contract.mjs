...
assert(js.includes('&& checks.socketStableInHand') && js.includes('&& checks.socketQuaternionStableInHand') && !js.includes('&& checks.visibleConfiguredGripAtHand') && !js.includes('&& checks.visibleAppliedHiltAtHand') && js.includes('const hiltAnchorSane = checks.appliedHiltPinnedToAuthoredSocket || (readyClipUsesScopedHiltTarget && checks.clipScopedHiltTargetVisible)') && js.includes('&& hiltAnchorSane') && js.includes('&& checks.readyHandOrientationSane') && js.includes('&& checks.visibleAppliedHiltMarker'), 'visual follow pass/fail should require FK-local weapon hierarchy stability plus sane visible Ready hand/grip orientation');
