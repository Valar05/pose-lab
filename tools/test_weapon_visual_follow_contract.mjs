...
assert(js.includes('configuredGripScreenMetricPresent') && js.includes('appliedHiltScreenMetricPresent') && js.includes('appliedHiltPinnedToAuthoredSocket') && js.includes('clipScopedHiltTargetVisible') && js.includes('socketStableInHand') && js.includes('socketQuaternionStableInHand') && js.includes('displayQuaternionStableInSocket'), 'visual follow should check FK-local position and rotation stability plus Ready scoped hilt target visibility');
assert(js.includes('handLocalGripOffsetVisible') && js.includes('appliedHiltAwayFromRawHand') && js.includes('readyHandOrientationSane'), 'visual follow should fail Ready screenshots where the grip basis or hilt collapses onto the raw wrist');
...
assert(js.includes('&& checks.socketStableInHand') && js.includes('&& checks.socketQuaternionStableInHand') && !js.includes('&& checks.visibleConfiguredGripAtHand') && !js.includes('&& checks.visibleAppliedHiltAtHand') && js.includes('const hiltAnchorSane = checks.appliedHiltPinnedToAuthoredSocket') && !js.includes('readyClipUsesScopedHiltTarget && checks.clipScopedHiltTargetVisible') && js.includes('&& hiltAnchorSane') && js.includes('&& checks.readyHandOrientationSane') && js.includes('&& checks.visibleAppliedHiltMarker'), 'visual follow pass/fail should require FK-local weapon hierarchy stability plus authored hilt pinning and sane visible Ready hand/grip orientation');
