...
assert(data.source?.attachmentSnapshots?.meshy?.attachment?.gripLocalPosition?.join(',') === '0.69507,-0.02421,-0.06231', 'Meshy snapshot should preserve the user hilt oracle candidate');
...
assert(profiles.includes('gripLocalPosition: [0.69507, -0.02421, -0.06231]'), 'solver must not replace the Meshy user hilt oracle placement');
