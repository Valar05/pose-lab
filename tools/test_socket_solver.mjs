...
assert(data.source?.attachmentSnapshots?.meshy?.attachment?.gripLocalPosition?.join(',') === '0.73272,0.0091,-0.01674', 'Meshy snapshot should preserve the visible mesh hilt oracle');
...
assert(profiles.includes('gripLocalPosition: [0.73272, 0.0091, -0.01674]'), 'solver must not replace the Meshy visible hilt placement');
