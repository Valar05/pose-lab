...
assert(data.source?.attachmentSnapshots?.meshy?.attachment?.gripLocalPosition?.join(',') === '0.6535,-0.02302,-0.07317', 'Meshy snapshot should preserve the T-pose-authored hilt oracle');
...
assert(profiles.includes('gripLocalPosition: [0.6535, -0.02302, -0.07317]'), 'solver must not replace the Meshy T-pose-authored hilt placement');
