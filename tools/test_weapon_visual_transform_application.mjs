...
async function main() {
  ...
  assert(JSON.stringify(config.proxy.rotationDeg || []) === JSON.stringify([0, 0, 0]), `Meshy socket orientation should remain identity for boring FK, got ${JSON.stringify(config.proxy.rotationDeg)}`);
  assert(JSON.stringify(config.attachment.rotationDeg || []) === JSON.stringify([-67.582, 76.718, -30.52]), `Meshy mesh-layer orientation should preserve the shared FK calibration, got ${JSON.stringify(config.attachment.rotationDeg)}`);
