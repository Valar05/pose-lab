...
assert(source.includes('FK close-up') && source.includes('yellow=direct FK WeaponGrip'), 'offline renderer must draw a screenshot-comparable FK close-up panel with unambiguous pure-FK labels');
...
assert(artifact.generatedClipStats?.droppedInitialRestKey === true && artifact.generatedClipStats?.weaponTrackEnabled === false, `generated ready must drop rest key but not emit normal weapon tracks: ${JSON.stringify(artifact.generatedClipStats)}`);
assert(artifact.generatedClipStats?.weaponTrackTarget === null, `generated ready must not target WeaponR or WeaponGrip in normal clips: ${JSON.stringify(artifact.generatedClipStats)}`);
assert(artifact.generatedClipStats?.weaponOrientationMode === null, `generated ready should not frame-solve weapon orientation for normal pure-FK clips: ${JSON.stringify(artifact.generatedClipStats)}`);
...
assert(artifact.sampleData.every((sample) => sample.weapon?.rightHand && sample.weapon?.socketHandBaseline && sample.weapon?.palmTarget && sample.weapon?.socket && sample.weapon?.appliedHilt && sample.weapon?.tip), 'each sample should include raw hand, hand-baseline, palm-target, and weapon landmarks');
...
assert(artifact.sampleData.every((sample) => sample.weaponPinning?.local?.socketInHand && sample.weaponPinning?.local?.socketQuaternionInHand), 'each sample should include WeaponGrip local transform under RightHand');
