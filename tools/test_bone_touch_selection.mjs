...
assert(js.includes('bodyBoneViewportPickingEnabled()') && js.includes("return this.activePanel === 'pose';"), 'normal body bone viewport picking should be available only in Pose mode');
assert(js.includes('if (!this.bodyBoneViewportPickingEnabled()) return [];'), 'ray-picked body bone handles should be disabled outside Pose mode');
assert(js.includes('if (!this.bodyBoneViewportPickingEnabled()) return null;'), 'screen-space body bone picking should be disabled outside Pose mode');
assert(js.includes("if (nextPanel !== 'pose') this.cancelAllTouchPoseGestures(null, false);"), 'leaving Pose mode should cancel active body-bone touch gestures');
