...
assert(evidence.visibilityGate?.status === 'fail', 'offline visibility gate should fail for current Ready clip');
assert(evidence.visibilityGate?.visibilityClass === 'weapon-hidden', 'offline visibility gate should model current Ready weapon visibility as hidden');
assert(evidence.transformGate?.status === 'blocked', 'transform gate should be blocked until visibility passes');
assert(evidence.transformGate?.transformClass === 'sword-follows-fk', 'offline transform measurements should remain separate from visibility');
assert((evidence.promotionVerdict || evidence.parity)?.visualVerdict === 'red', 'current parity gate should be red until machine truth gates pass');
...
assert(artifact.visibilityGate?.status === 'fail', 'parity artifact should expose the visibility gate');
assert(artifact.transformGate?.status === 'blocked', 'parity artifact should expose the blocked transform gate');
assert(offline.result === artifact.result, 'gate result should mirror parity artifact result');
assert((artifact.promotionVerdict || artifact.parity)?.visualVerdict === 'red', 'infrastructure check should preserve the current red build');
