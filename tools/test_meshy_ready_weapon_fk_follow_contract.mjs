...
assert(['sword-rest-space', 'sword-follows-fk'].includes(artifact?.offlineTruth?.visualClass), `unexpected offline visual class ${artifact?.offlineTruth?.visualClass}`);
...
for (const key of ['generatedReadyClipResolved', 'readyClipSampled', 'readyHandDisplacedFromRest', 'noGeneratedWeaponTracks', 'manualSaberPlacementPreserved']) {
...
if (artifact?.parity?.visualVerdict !== 'fixed') failures.push(`RED BUILD: offline/web truth parity not fixed: observed=${artifact?.observedWebTruth?.visualClass} offline=${artifact?.offlineTruth?.visualClass} parityFailure=${artifact?.parity?.parityFailure || 'current-visual-truth-red'}`);
