...
const LAB_BUILD = 'meshy-ready-golden-fk';
    ...
    for (const [targetKey, target] of this.actors) {
      ...
      for (const spec of specs) {
        ...
        const customOriginPrefix = spec.originPrefix || (spec.retargetMode === 'weapon-path-ik'
          ...
          : spec.retargetMode === 'mapped-rotation' || spec.retargetMode === 'position-guided-arm' || spec.retargetMode === 'fps-upper-key-convert' || spec.retargetMode === 'meshy-fps-visual-ik-ready'
            ? 'retarget:' + sourceKey + '->' + targetKey + ':' + mappedTag
            : '');
